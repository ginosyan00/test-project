...
export const Navbar: React.FC = () => {
  ...
  const changeLang = (lng: 'en' | 'hy' | 'ru') => {
    ...
    if (import.meta.env.DEV) {
      ...
      console.log('%c[lang] -> ' + lng, 'color:#3b82f6');
    }
  };
...
function linkCls(active: boolean) {
  return `hover:text-gray-600 ${active ? 'text-gray-900 font-medium' : 'text-gray-700'}`;
}

function langBtnCls(active: boolean) {
  return `hover:text-gray-800 ${active ? 'text-gray-900 font-semibold' : 'text-gray-600'}`;
}
